...
- ~~**New-session working directory**~~ **RESOLVED (settled post-v1 after two iterations):** spatial targeting. Each directory group header grows hover buttons (✳ / ⌨) that spawn a session of that type *in that directory* — one click, target visible. The toolbar + menu (two items, Claude/shell) is for *new* directories only: it always opens the OS folder picker (which remembers the last pick). A recents-list variant was built and deleted — group headers made it redundant.
