...
- **Hooks are blind to user interrupts (found + fixed 2026-07-12):** documented behavior — `Stop` fires only on normally-completed turns, and *no* hook fires when a permission/question dialog is dismissed (Esc) or a running turn is interrupted, so dots stuck amber/red after a cancel. Fix: the renderer *observes* the keystrokes it already forwards to the PTY (bytes pass through untouched — observation, not steering, same spirit as the title): a lone Esc (`0x1b` alone; arrows etc. arrive as longer chunks) or Ctrl+C while `running`/`waiting` → `idle`; Enter while `waiting` → `running` (approve and deny-with-feedback both resume the turn). Optimistic nudge only — the next hook event stays authoritative. Parked, verify payloads empirically before touching: the newer `PermissionRequest` hook event could make `waiting` more precise than blanket `Notification`, whose payload types include `idle_prompt` (a potential false amber under the blanket map).
