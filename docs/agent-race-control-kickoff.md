...
- ~~**New-session working directory**~~ **RESOLVED (Phase 4, extended post-v1):** the + menu is two-step — pick the session type, then pick from the 8 most recent directories (persisted) or Browse… for the OS folder picker (which remembers the last-picked dir as its start point). With no recents yet, the picker opens directly.
