...
- **Status dot colors (settled 2026-07-12, replacing CI-badge semantics):** traffic lights read from the **user's** point of view — every color answers "is this session mine to act on?" Claude sessions: **running = red** (the agent is driving, hands off), **waiting = amber, pulsing** (it's asking for you), **idle = green** (your turn — a fresh session starts here, at its prompt). **A live shell is neutral full-strength ink (`fg`)** — a power LED, not an agent state: "running" only means its PTY is alive (we can't and won't inspect the foreground process), and green stays exclusive to "a Claude awaits you". **exited = faded muted gray** for both types — a shell's dot literally just dims when it dies. All hexes are existing Primer tokens (`danger`/`attention`/`success`/`fg`) with roles remapped — zero new colors. Accepted trade-offs: a tower of busy agents glows red (correct under this framing — all stop lights, nothing for you), and red is spent on "busy", so a hypothetical bad-exit state can never claim it (fine — don't over-model status).
...
- ~~**New-session working directory**~~ **RESOLVED (settled post-v1 after two iterations):** spatial targeting. Each directory group header grows hover buttons (✳ / ⌨, presented as the header's segmented cluster — see *Directory groups*) that spawn a session of that type *in that directory* — one click, target visible. The toolbar + menu (two items, Claude/shell) is for *new* directories only: it always opens the OS folder picker (which remembers the last pick). A recents-list variant was built and deleted — group headers made it redundant.
