...
- `npm run dist` — build + NSIS installer into `dist/` (`npm run dist:dir` for the unpacked smoke-test build; `npx electron scripts/make-icon.mjs` regenerates `build/icon.ico` from the same SVG the app rasterizes (`src/renderer/src/assets/arc.svg`))
